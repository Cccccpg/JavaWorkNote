...
7. 如果线程A先执行完原本区间的迁移工作，就会到`transferindex `的位置，又会继续上面的过程，继续执行扩容迁移操作，直到transferindex的值等于0结束，才表示这个线程的扩容操作已经结束。每一个线程走到`transferindex = 0`的位置，就会让sizeCtl的值+1。
